package com.meby98.framework.specification;

import com.mercadona.supplychain.lib.common.StringUtils;
import com.mercadona.supplychain.lib.common.dto.DateOperatorQuery;
import com.mercadona.supplychain.lib.common.specification.exceptions.DateOperatorWrongException;
...
public abstract class BaseSpecification<E> {
    ...
    protected Predicate buildLikeLowerCase(final Path<String> path, @NotNull final String value,
                                           final CriteriaBuilder builder) {
        return builder.like(builder.lower(path), "%" + value.toLowerCase() + "%");
    }

    protected Predicate buildLikeUnaccent(final Path<String> path, @NotNull final String value,
                                          final CriteriaBuilder builder) {
        return builder.like(builder.function(UNACCENT_POSTGRESQL_FUNCTION, String.class
                , path), "%" + StringUtils.removeAccents(value) + "%");
    }

    protected Predicate buildLikeUnaccentAndLowerCase(final Path<String> path, @NotNull final String value,
                                                      final CriteriaBuilder builder) {
        return builder.like(builder.function(UNACCENT_POSTGRESQL_FUNCTION, String.class
    ...
    protected <T> Path<T> getObjectPath(@NotNull final Path<?> root, final String field) {
